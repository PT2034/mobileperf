...
def question_calc_request(raw_question_context):
...
def build_question_check_url(raw_question_context):
    # raw_question_context= pretreat_q(raw_question_context)
...
def contract_calc_res_collector(questionDict):
    full_time_timestamp = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    calc_res_title = ["status_code", "code", "msg", "trace_id", "origin_question"]
    resFile = 'result/contract_qlist_res_%s.csv' % full_time_timestamp
    ...
    for i in range(0, len(questionDict)):
        try:
            with open(res_file, 'a+') as df:
                csv.writer(df, lineterminator='\n').writerow(question_calc_request(questionDict[i]))
        except RuntimeError as e:
            logger.error(e)
...
def read_questions():
    qDict = []
    try:
        with open('input/contract_qlist.csv', 'r') as f2:
            qList = f2.readlines()
            if (len(qList) != 0):
                for i in range(0, len(qList)):
                    qDict.append(qList[i].replace("\n", ""))
...
if __name__ == '__main__':
    ...
    qDict = read_questions()
    contract_calc_res_collector(qDict)
