...
def batch_execute_command(cmds):
    if cmds:
        cmdList = cmds.split(';')
        if len(cmdList)>=1:
            for c in cmdList:
...
def execute_pipeline_command(cmds):
    res = os.popen(cmds).read()
    print('execute_pipeline_command, curr commands=',cmds)
    return res
...
if __name__ == "__main__":
    # print(batch_execute_command("pwd;ls"))
    print(execute_pipeline_command("echo 'hello world' |wc -c"))